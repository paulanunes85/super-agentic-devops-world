...
```mermaid
flowchart TD
    A["1. DEFINIR A SPEC\n-----------------\nHumano escreve\nespecificacao detalhada"] --> B["2. REVISAR A SPEC\n-----------------\nTime revisa\na especificacao"]
    B --> C["3. AGENTE GERA CODIGO\n-----------------\nCoding Agent cria\nimplementacao"]
    C --> D["4. VALIDACAO AUTOMATIZADA\n-----------------\nTestes, linting,\ntype checking"]
    D --> E{"Passou na\nvalidacao?"}
    E -->|Sim| F["5. REVISAO HUMANA\n-----------------\nDev revisa contra\na intencao da spec"]
    E -->|Nao| G["Agente corrige\nautomaticamente"]
    G --> D
    F --> H{"Atende\na spec?"}
    H -->|Sim| I["MERGE + DEPLOY"]
    H -->|Nao| J["6. ITERAR\n-----------------\nRefinar spec\ne re-gerar"]
    J --> A
    ...
    style I fill:#43B047,stroke:#333,color:#fff
...
sequenceDiagram
    ...
    participant Team as Time/Reviewers
    participant Agent as Coding Agent
    participant CI as CI/CD Pipeline
    participant Prod as Producao

    Dev->>GH: 1. Cria Issue com Spec detalhada
    Dev->>GH: 2. Abre PR da Spec para review
    Team->>GH: 3. Revisa e aprova a Spec
    Dev->>Agent: 4. Atribui Issue ao Coding Agent
    Agent->>Agent: 5. Le spec + analisa codebase
    Agent->>GH: 6. Cria branch + implementa + abre PR
    CI->>CI: 7. Roda testes, lint, type check
    CI->>GH: 8. Reporta status dos checks
    Dev->>GH: 9. Revisa PR contra a spec
    Dev->>GH: 10. Aprova e faz merge
    GH->>Prod: 11. Deploy automatico
    Dev->>GH: 12. Fecha issue, move spec para completed
```
