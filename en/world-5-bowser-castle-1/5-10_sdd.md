...
```mermaid
flowchart TD
    A["1. DEFINE THE SPEC\n-----------------\nHuman writes\ndetailed specification"] --> B["2. REVIEW THE SPEC\n-----------------\nTeam reviews\nthe specification"]
    B --> C["3. AGENT GENERATES CODE\n-----------------\nCoding Agent creates\nimplementation"]
    C --> D["4. AUTOMATED VALIDATION\n-----------------\nTests, linting,\ntype checking"]
    D --> E{"Passed\nvalidation?"}
    E -->|Yes| F["5. HUMAN REVIEW\n-----------------\nDev reviews against\nspec intent"]
    E -->|No| G["Agent fixes\nautomatically"]
    G --> D
    F --> H{"Meets\nthe spec?"}
...
sequenceDiagram
    ...
    Dev->>GH: 1. Creates Issue with detailed Spec
    Dev->>GH: 2. Opens PR for Spec review
    Team->>GH: 3. Reviews and approves the Spec
    Dev->>Agent: 4. Assigns Issue to Coding Agent
    Agent->>Agent: 5. Reads spec + analyzes codebase
    Agent->>GH: 6. Creates branch + implements + opens PR
    CI->>CI: 7. Runs tests, lint, type check
    CI->>GH: 8. Reports check status
    Dev->>GH: 9. Reviews PR against the spec
    Dev->>GH: 10. Approves and merges
    GH->>Prod: 11. Automatic deploy
    Dev->>GH: 12. Closes issue, moves spec to completed
```
