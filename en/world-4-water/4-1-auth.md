...
|--------|-------------|---------------|
| **Duration** | Short (15min - 1h) | Long (7 - 30 days) |
| **Usage** | Access protected resources | Generate new Access Tokens |
| **Storage** | Memory or httpOnly cookie | Secure httpOnly cookie |
| **If stolen** | Limited damage (expires quickly) | More dangerous (revoke immediately) |
| **Mario** | Daily pass for the castle | Annual card that renews the pass |
...
### 3.5 Code Example: Generating a JWT

```javascript
const jwt = require('jsonwebtoken');
const SECRET = process.env.JWT_SECRET; // NEVER hardcode the secret!
...
## 4. OAuth 2.0: Asking the King to Confirm Your Identity

### 4.1 The Problem OAuth Solves

Imagine you want to access a new service — for example, a task management app. Without OAuth, you would need to create YET another account with email and password. With OAuth, you can ask Google, Microsoft, or GitHub to confirm who you are.

> **Mario Analogy**: OAuth is like Mario asking the **King of the Mushroom Kingdom (Google/Microsoft)** to confirm his identity. Instead of Mario proving who he is at every castle he visits, he says: *"Ask the King! He knows me!"* The King confirms: *"Yes, that is Mario. You can trust him."*

### 4.2 The 4 Roles of OAuth

| Role | Description | Mario Analogy |
|------|-------------|---------------|
| **Resource Owner** | The user who owns the data | Mario (profile owner) |
| **Client** | The application that wants to access the data | TodoApp (wants to know who Mario is) |
| **Authorization Server** | Who issues access tokens | King of the Mushroom Kingdom (Google) |
| **Resource Server** | Where the protected data is stored | Castle with Mario's profile |

### 4.3 The Authorization Code Flow

The most secure and recommended flow for web applications:

```
1. Mario clicks "Login with Google" in TodoApp
2. TodoApp redirects Mario to Google
3. Google asks: "TodoApp wants to access your name and email. Allow?"
4. Mario clicks "Allow"
5. Google redirects back to TodoApp with a temporary CODE
6. TodoApp sends this code to Google (server-to-server)
7. Google returns an Access Token
8. TodoApp uses the token to fetch Mario's name and email
9. TodoApp creates session/JWT for Mario
```

### 4.4 OAuth Flow Diagram
